...
export default {
	...
	computed: {
		currentStock() {
			...
			return this.selectedSize ? this.getStockBySize(this.selectedSize) : 0;
		},
		isAddDisabled() {
		...
		product: {
			...
			handler(val) {
				...
				if (val.category === 'Clothing') {
					this.selectedSize = this.availableSizes.find(s => this.getStockBySize(s) > 0) || null;
				}
			}
		},
		selectedSize() {
	...
	methods: {
		...mapActions(useMerchStore, ['purchaseProduct']),
		getStockBySize(size) {
			return this.product.stockBySize ? this.product.stockBySize[size] : 0;
		},
		addToCartHandler() {
			if (!this.isAddDisabled) {
				this.purchaseProduct(this.product, this.amount, this.selectedSize);
				this.closeAndReset();
			}
		},
		resetSelection() {
